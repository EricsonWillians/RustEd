...
use crate::map::{LineDef, Sector, SideDef, Thing, Vertex}; // Import from the new map module
...
impl Document {
    ...
    pub fn move_vertex(&mut self, vertex_id: usize, new_x: i32, new_y: i32) -> Result<(), String> {
        ...
        if let Some(vertex) = vertices.get_mut(vertex_id) {
            let vertex_ref = Arc::make_mut(vertex); // Get a mutable reference to the Vertex
